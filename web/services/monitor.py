    ...
    def _initialize_data(self):
        ...
        for symbol in self.symbols:
            try:
                ...
                if not klines.empty:
                    with self.data_lock:
                        self.key_levels[symbol] = self._calculate_key_levels(
                            klines
                        )
    ...
    def _get_klines(
        ...
        try:
            ...
            response = requests.get(
                endpoint, params=params, proxies=self.proxies, timeout=10
            )
    ...
    def _calculate_key_levels(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for level in sorted(support_candidates):
                if level < latest_close and (
                    not support_levels
                    or abs(level - support_levels[-1]) / latest_close > 0.002
                ):
            ...
            for level in sorted(resistance_candidates):
                if level > latest_close and (
                    not resistance_levels
                    or abs(level - resistance_levels[-1]) / latest_close
                    > 0.002
                ):
            ...
            try:
                for symbol in self.symbols:
                    ...
                    klines = self._get_klines(
                        symbol.upper(), interval='1h', limit=500
                    )
                    if not klines.empty:
                        with self.data_lock:
                            self.key_levels[
                                symbol
                            ] = self._calculate_key_levels(klines)
        ...
        try:
            ...
            if len(self.volume_buffers[symbol]) >= 5:
                ...
                recent_bid_volume = sum(
                    v['bid_volume'] for v in volume_data[-5:]
                )
                recent_ask_volume = sum(
                    v['ask_volume'] for v in volume_data[-5:]
                )
